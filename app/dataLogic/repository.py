    ...
    def updateitems(self,uid):
        ...
        try:
            ...
            for symbol in symbols:
                sql="select t1.timestamp,t2.price,t2.num,t1.strategy from marketorder t1 join marketorderitem t2 on t1.oid=t2.oid where t1.uid='%s' and symbol='%s'"%(uid,symbol)
                data=self.cursor.execute(sql).fetchall()
                data=[da for da in data]
